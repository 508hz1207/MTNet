...
# returns (x, y)
def get_points_and_scores_from_mask(mask, threshold):#mask=1024,1024 threshold=1.275
    rcs = np.column_stack(np.where(mask > threshold))#10139,2 elect predicted score >threshold ,as the elected keypoint.coordinate shpe is (y=height,x=width)
    xys = rcs[:, ::-1]#coordinate type is (x,y)(pixel coordinate)#10139,2
    scores = mask[mask > threshold]#elect the corresponding scores  10139,
    return xys, scores
...
#elect key point as the graph_points
def extract_graph_points(keypoint_mask, road_mask, config):
    kp_candidates, kp_scores = get_points_and_scores_from_mask(keypoint_mask, config.ITSC_THRESHOLD * 255)#kp_candidates=10139,2  kp_scores=10139,
    kps_0 = nms_points(kp_candidates, kp_scores, config.ITSC_NMS_RADIUS)#10139,2
    kp_candidates, kp_scores = get_points_and_scores_from_mask(road_mask, config.ROAD_THRESHOLD * 255)#kp_candidates=40183,2 kp_scores=40183,
    kps_1 = nms_points(kp_candidates, kp_scores, config.ROAD_NMS_RADIUS)#40183,2
    # prioritize intersection points
    kp_candidates = np.concatenate([kps_0, kps_1], axis=0)#50322,2
    kp_scores = np.concatenate([np.ones((kps_0.shape[0])), np.zeros((kps_1.shape[0]))], axis=0)#he qi lai hou ,zai xuan  #50322,
    kps = nms_points(kp_candidates, kp_scores, config.ROAD_NMS_RADIUS)#154,2
    return kps

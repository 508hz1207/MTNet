...
def igraph_from_adj_dict(graph, coord_transform):
    ...
    nodes, edges = convert_from_sat2graph_format(graph)#edges有180个，有重复的边，AB和BA事一样的
    n_vertices = nodes.shape[0]
    if n_vertices == 0:
        nodes = np.zeros((0, 2), dtype=nodes.dtype)
    edges = set([(min(src, tgt), max(src, tgt)) for src, tgt in edges])#得到90个互不重复的边，set取出唯一值，并创建一个set对象。不能重复
    g = ig.Graph(n_vertices, list(edges))#xy顶点和边的逻辑关系
    try:
        g.vs['point'] = coord_transform(nodes)
        a= coord_transform(nodes)# to
    except Exception:
        print("==================")
        print(nodes.shape)
        print(nodes)
...
def find_crossover_points(graph):#找到两直线的完全交叉点
    # takes igraph
    ...
    edges = graph.es#返回所有边的集合
    lines = [(points[edge.source], points[edge.target]) for edge in edges]#拿到所有边对应的起始坐标对
    line_bboxes = [get_line_bbox(line) for line in lines]#得到每个边的包围框
...
def subdivide_graph(graph, resolution):#对所有的边进行内插，得到更多的点和边，原来123个点，126个边，内插后新图1296个点，1299个边
    # takes igraph
    new_points = [p for p in graph.vs['point']]#拿到所有点的坐标组合
    new_edges = []
    for edge in graph.es:
        p0, p1 = graph.vs['point'][edge.source], graph.vs['point'][edge.target]
        length = np.linalg.norm(p1 - p0) #计算两点间的距离
        sample_pieces = max(1, int(length / resolution))
        # [N, ]
        samples = np.linspace(0.0, 1.0, sample_pieces + 1, endpoint=True)
        # [N, 2] = [1, 2] + [N, 1] @ [1, 2]
        sampled_pts = np.expand_dims(np.array(p0), axis=0) + np.expand_dims(samples, axis=1) @ np.expand_dims(p1 - p0, axis=0)#N,2
        # [N-2, 2]
        sampled_pts = sampled_pts[1:-1, :] # [N-2, 2],始末点不取
        new_point_indices = []
...
def nms_points(points, scores, radius, return_indices=False):#points=10139,2, scores=10139,, radius=8
    # if score > 1.0, the point is forced to be kept regardless
    sorted_indices = np.argsort(scores)[::-1]#10139,
    sorted_points = points[sorted_indices, :]#10139,2
    sorted_scores = scores[sorted_indices]#10139,
    kept = np.ones(sorted_indices.shape[0], dtype=bool)#10139,

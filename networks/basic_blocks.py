...
class DBlock(nn.Module):
    def __init__(self, channel):
        ...
        self.dilate1 = nn.Conv2d(
            channel, channel, kernel_size=3, stride=1,dilation=1, padding=1)
        self.dilate2 = nn.Conv2d(
            channel, channel, kernel_size=3,stride=1, dilation=2, padding=2)
        self.dilate3 = nn.Conv2d(
            channel, channel, kernel_size=3, stride=1,dilation=4, padding=4)
        self.dilate4 = nn.Conv2d(
            channel, channel, kernel_size=3,stride=1, dilation=8, padding=8)
...
class DecoderBlock1DConv4(nn.Module):
    def __init__(self, in_channels, n_filters):
        ...
        self.deconv1 = nn.ConvTranspose2d(
            in_channels // 4, in_channels // 8, (1, 9), padding=(0, 4)
        )#in=128,out=64 2.64,32 3.32 16 4.16 8
        self.deconv2 = nn.ConvTranspose2d(
            in_channels // 4, in_channels // 8, (9, 1), padding=(4, 0)
        )#in=128,out=64 2.64,32 3. 32 16 4.16 8
        self.deconv3 = nn.ConvTranspose2d(
            in_channels // 4, in_channels // 16, (9, 1), padding=(4, 0)
        )#in=128,out=32 2.64,16 3.32 8 4.16 4

...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.,channels_first=False):
        ...
        Linear = Linear2d if channels_first else nn.Linear# Linear = =F.conv2d(x, self.weight[:, :, None, None], self.bias)
    ...
    def forward(self, x):
        x = self.fc1(x)#run 6,96,64,64->6,384,64,64 Linear =F.conv2d(x, self.weight[:, :, None, None], self.bias)
        x = self.act(x)#GElu 6,384,64,64->6,384,64,64
        x = self.drop(x)# 6,384,64,64->6,384,64,64
        x = self.fc2(x)# 6,384,64,64->6,96,64,64
        x = self.drop(x)#6,96,64,64->6,96,64,64
        return x
...
class SS2Dv0:
    ...
    def forwardv0(self, x: torch.Tensor, seq=False, force_fp32=True, **kwargs):
        ...
        x=x.permute(0,2,3,1).contiguous()#6,96,64,64->6,64,64,96
        t=x
        x=x.view(-1,c).contiguous()#6,64,64,96->6x64x64=24576,96
        x = self.in_proj(x)#24576,96->24576,192
        p_t=self.in_proj(t)#6,64,64,96->6,64,64,192
        x=x.view(b,h,w,c*2).contiguous()#24576,192->6,64,64,192
        judge=torch.equal(x,p_t)#True
        x, z = x.chunk(2, dim=-1) # (b, h, w, d)splite two chunk,shape is 6,64,64,96
        z = self.act(z)#Silu,6,64,64,96->6,64,64,96
        x = x.permute(0, 3, 1, 2).contiguous()#6,64,64,96->6,96,64,64
        x = self.conv2d(x) # (b, d, h, w)#6,96,64,64->6,96,64,64
        x = self.act(x)#Silu,6,64,64,96->6,64,64,96
        selective_scan = partial(selective_scan_fn, backend="mamba")
        
        B, D, H, W = x.shape#,6,96,64,64
        D, N = self.A_logs.shape#384,1
        K, D, R = self.dt_projs_weight.shape#4,96,6
        L = H * W#4097
        ...
        wh_y = torch.transpose(out_y[:, 1].view(B, -1, W, H), dim0=2, dim1=3).contiguous().view(B, -1, L)#6,4,96,4096->6,96,4096
        invwh_y = torch.transpose(inv_y[:, 1].view(B, -1, W, H), dim0=2, dim1=3).contiguous().view(B, -1, L)#6,2,96,4096->6,96,4096
        # 四种状态叠加
    ...
    def _forward(self, input: torch.Tensor):
        ...
        if self.ssm_branch:
            if self.post_norm:
                ...
                x = x + self.drop_path(self.op(self.norm(x)))#run 6,96,64,64->6,96,64,64
        if self.mlp_branch:
            if self.post_norm:
                x = x + self.drop_path(self.norm2(self.mlp(x))) # FFN
            else:
                x = x + self.drop_path(self.mlp(self.norm2(x))) # FFN#run#run 6,96,64,64->6,96,64,64
        return x
...
class VSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.classifier = nn.Sequential(OrderedDict(
            norm=norm_layer(self.num_features), # B,H,W,C   768
    ...
    def forward(self, x: torch.Tensor):
        x = self.patch_embed(x)#6,3,256,256->6,96,64,64
        if self.pos_embed is not None:#not run
            pos_embed = self.pos_embed.permute(0, 2, 3, 1) if not self.channel_first else self.pos_embed
            x = x + pos_embed
        for layer in self.layers:#run
            x = layer(x)#6,96,64,64->6,192,32,32->6,383,16,16->6,768,8,8
        x = self.classifier(x)#6,768,8,8->6,1000
        return x
...
if __name__ == "__main__":
    ...
    def bench(model):
        ...
        inp = torch.randn((6, 3, 256, 256)).cuda()
        for _ in range(30):
            model(inp)
